...
function showNotification(message, type = 'success') {
    ...
    if (messageElement) messageElement.tex
    tContent = message;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const initUserSession = () => {
     ...
     const initShoppingCart = () => {
        ...
        if (proceedToCheckoutBtn) proceedToCheckoutBtn.addEventListener('click', e => { if (cart.items.length === 0) { e.preventDefault(); showNotification('Tu carrito está vacío.', 'error'); } saveCart(); });
